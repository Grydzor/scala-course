...
object HomeTask extends App {
  ...
  def tryGetSubscribers(isRisky: Boolean, source: String): Try[Seq[SubscriberInfo]] = {
    Try(getFile(isRisky, source)) map (bufferedSource => functional.usingSourceWithFinally(bufferedSource)(source => toSubscribers(source))(println("execution finally")))
  }
  ...
  def enrichAndSend(getFileIsRisky: Boolean,
                    ...
                    fileSource: String): Either[Error, Int] = ((for {
