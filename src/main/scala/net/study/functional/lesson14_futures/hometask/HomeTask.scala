package net.study.functional.lesson14_futures.hometask
...
object HomeTask extends App {
  ...
  case class EmptyException(innerMessage: String) extends Exception

  val dbSeq: Seq[Int => Future[Option[Account]]] = Seq(getAccountInfoDb1, getAccountInfoDb2)
  val paymentSeq: Seq[String => Future[Option[Seq[PaymentInfo]]]] = Seq(getPaymentInfo, getPaymentInfo2, getPaymentInfo3)

  def getFromDBs[IN, OUT](in: IN, seqAcc: Seq[IN => Future[Option[OUT]]]): Future[OUT] = {

    def get(accs: Seq[IN => Future[Option[OUT]]]): Future[OUT] = {
      if(accs.isEmpty) return Future.failed(EmptyException("empty"))
      accs.head(in).flatMap {
        case Some(value: OUT) => Future.successful(value)
        case None => get(seqAcc.tail)
      }
    }
  ...
  val accountsF: Seq[Future[Account]] = accountIds.map(id => getFromDBs(id, dbSeq))
  val paymentF: Seq[Future[Seq[PaymentInfo]]] = accountsF.map(fut => fut.flatMap(account => getFromDBs(account.login, paymentSeq)))
